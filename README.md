...
[下载最新安装包](https://github.com/bailigebai/legado.koplugin/releases/latest) · [下载 v0.10.28 ZIP](https://github.com/bailigebai/legado.koplugin/releases/download/v0.10.28/legado.koplugin-v0.10.28-20260924.zip)

应用商店：刷新仓库列表，打开 `bailigebai/legado.koplugin`，选择“下载插件”并安装最新正式版，完成后重启 KOReader。

2026-09-24 v0.10.28：新增“Swipe动画”，单独保留原项目的条带擦除与帧后等待顺序：默认竖屏 8 段、20ms，横屏 6 段、10ms，按设备像素对齐，正反翻页镜像推进，支持 UI/Fast。与已有异步“擦除渐显”并列，在同一次绘制任务中完成全部条带，避免后台任务插入动画帧之间。仅提取阅读页效果，不安装原项目的全局补丁；原版硬件动画、侧边水波纹默认及其他效果保留。

本版修复跨章画面不完整：宿主仅重画页脚或菜单区域、打断尚未完成的动画时，取消旧帧后会补交完整阅读区域的刷新；上层菜单先完成绘制再一起提交，既补全下一章，也不会被迟到动画覆盖。正常完成的动画仍不追加整页刷新。

Swipe 操作：阅读菜单 → 阅读设置 → 动画效果 → Swipe动画；在同一界面点击“竖屏帧延迟 / 横屏帧延迟”调整，可选 0 / 10 / 20 / 30 / 40 / 50 / 80ms，并按书保存。这个入口不依赖 KOReader 全局 Swipe 开关，所以即使宿主菜单的延迟项呈灰色，也可在插件内调整。没有修改宿主菜单或其全局设置。帧延迟是在每次条带提交之后等待；默认竖屏共等待约 140ms、横屏约 50ms，另加绘制和设备提交时间。动画执行期间界面线程会短暂被占用，这是所参考上游顺序的取舍，不承诺物理零延迟。显式开启“跨章净屏动画”时，跨章仍优先净屏；对比 Swipe 同章/跨章手感时请保持该开关关闭。

验收：重启 KOReader，选择 Swipe动画，分别连续前翻、后翻、翻到下一章；再对现有水波纹测试翻页途中打开菜单/目录，确认无上一章残留、下一章首屏完整。实机刷新速度、残影及背景图仍需 Kindle 验收。
...
动画选择：阅读菜单 → 阅读设置 → 动画效果。点按依次切换：原版翻页、Swipe动画、擦除渐显、水波纹、侧边水波纹、聚拢水波纹、波浪推进、关闭。三种新增形态复用现有可取消调度、UI/Fast 刷新和横竖屏帧间隔；跨章净屏开关开启时，跨章优先执行净屏动画。
...
`legado.koplugin` 在 Kindle KOReader 上独立运行。v0.10.28 未激活时最多添加 5 本书，添加第 6 本需输入短密钥；同一授权同时解锁阅读小票和阅读回顾数据展示，其他功能免费。已有书籍不会因升级被删除，重复加入同一本不占新名额，删除收藏后可腾出名额。首次激活联网绑定设备，之后可离线验证；沿用已发放的 Legado 短密钥。新安装不内置书源，需要从本地 JSON 或网址自行导入。网络书默认使用基于 Leko 的独立文字阅读器和侧边水波纹，兼容缺少 Fast 刷新接口的 Kindle。本地 EPUB/PDF 等文档仍交给 KOReader；独立阅读菜单点击“关闭无感阅读”可回到 KOReader。双向切换保留章节和近似章内位置，两种排版分别按书保存。阅读页目录新增侧边懒加载面板，每页 15 章，可放在左侧或右侧，支持目录、书签、字体和配置页。本版继续优化缓存命中章节切换，当前章节提交后立即后台预取下一章，并继续填充后两章；页脚“阅读进度”可独立启用/停用并保留显示模式。详情见 [v0.10.22 修复记录](docs/reader-library-0.10.22.md)。
