...
[下载最新安装包](https://github.com/bailigebai/legado.koplugin/releases/latest) · [下载 v0.10.31 ZIP](https://github.com/bailigebai/legado.koplugin/releases/download/v0.10.31/legado.koplugin-v0.10.31-20260926.zip)

应用商店：刷新仓库列表，打开 `bailigebai/legado.koplugin`，选择“下载插件”并安装最新正式版，完成后重启 KOReader。

2026-09-26 v0.10.31：修复跨章首屏尚未显示时，再次翻页直接跳到第二页的问题。动画未结束时的提前点击会立即补全首屏，正常显示后下一次点击继续翻页；章内翻页和正常完成的动画不增加额外点击。宿主局部重画失败时，保留并显示已经生成的完整首屏，避免丢弃目标画面后残留旧正文或黑带。固定 Swipe 的效果、时间参数、预取和按书设置不变，同一设备升级无需重新激活。测试与实机边界见 [跨章首屏修复记录](docs/chapter-presentation-0.10.31.md)。

2026-09-24 v0.10.30：修复大书源合集被旧5MiB上限拒绝的问题。本地导入、显式下载和进程传输统一支持16MiB，普通阅读请求默认仍为4MiB。用户提供的15份原始JSON全部通过真实SQLite首次导入、重复导入、关闭重开和字段比对；与已有919源连续合并后为3331个不同站点地址，保留停用状态、书架和进度。原文件含重复站点，数量按地址去重。已保留固定Swipe动画和之前的功能，升级无需重新激活。逐份结果与实机边界见 [导入验证报告](docs/source-import-0.10.30.md)。

2026-09-24 v0.10.29：“Swipe动画”改为独立固定预设，使用上游默认竖屏 8 段/20ms、横屏 6 段/10ms、UI 刷新；不显示延迟、刷新模式与跨章净屏调节项，旧参数不影响此预设。仍按上游顺序在同一次绘制任务内完成对齐条带和帧后等待，正反翻页镜像推进。其他动画及其按书设置保留，不安装全局补丁。

本地书源导入改用 KOReader 文件选择器，长按 JSON 文件并确认即可导入；没有文件选择器的宿主保留手输入口，兼容路径首尾空白与成对引号。读文件失败、读取已有书源失败、保存数据库失败分别提示；误把网址当本地路径会提示改用“从网址导入”。失败不会清除已有书源，不更改书架、进度和授权。已用用户此前提供的 919 源 JSON 验证真实 SQLite 首次导入、重开逐字段核对、重复导入不重复、停用状态与书架进度保留。截图中的设备故障尚缺同次入口/日志，不能据截图认定空间不足，也未在 Kindle 重现。

本版修复跨章画面不完整：宿主仅重画页脚或菜单区域、打断尚未完成的动画时，取消旧帧后会补交完整阅读区域的刷新；上层菜单先完成绘制再一起提交，既补全下一章，也不会被迟到动画覆盖。正常完成的动画仍不追加整页刷新。

Swipe 操作：阅读菜单 → 阅读设置 → 动画效果 → Swipe动画，选中即用，无需调参。固定等待合计竖屏约 140ms、横屏约 50ms，另加绘制和设备提交时间。动画执行期间界面线程会短暂被占用，这是上游顺序的取舍，不承诺物理零延迟。此模式跨章仍使用 Swipe，不被之前保存的跨章净屏开关替代；切回其他动画时恢复原有调节项及其设置。

验收：重启 KOReader，进入“更多 → 书源管理 → 从本地 JSON 导入”，长按设备上的 JSON 并确认，导入后再次导入应显示更新而非新增。再选择 Swipe动画，确认不显示延迟项，连续前翻、后翻、翻到下一章。实机导入错误是否消失、刷新速度、残影及背景图仍需 Kindle 验收。

2026-09-23 v0.10.27：统一修正全部翻页效果的刷新衔接。动画完成不再追加整页刷新；复用阅读窗口跨章时不再先整页重画再执行动画；后台分页、预排版让步给正在进行的动画。水波纹只复制新露出的区域，波浪推进的起伏减半；保留原版、擦除及所有水波纹形态。增加 40ms 档位并将设置标为“帧间隔”，驱动提交耗时计入间隔。原生翻页提交后弹菜单不再重复刷页；旋转、快速连翻和调度失败均取消旧任务或回退正常绘制。

帧间隔选择：阅读菜单 → 阅读设置 → 竖屏帧间隔，可选 0 / 10 / 20 / 30 / 40 / 50 / 80ms；横屏同样可选。40ms 是相邻软件帧的目标间隔，竖屏 8 帧从首帧到末帧约 280ms，加上绘制与驱动等待；数值越大通常越慢，并非“更快”的档位。已有按书设置保持不变。“原版翻页”在支持的 Kindle 上使用 KOReader 驱动原生动画，其节奏由设备控制，不受软件帧间隔设置影响；其他设备使用与擦除相同的分区方式。跨章净屏有独立节奏，想减少黑白波动可保持关闭。

实现继续参考 [Swipe_Animation 的分区揭页](https://github.com/koplugin-swipe-animation/Swipe_Animation.koplugin) 和 [KOReader Kindle 驱动的原生动画接口](https://github.com/koreader/koreader-base/blob/master/ffi/framebuffer_mxcfb.lua)。不改宿主全局刷新策略。桌面测试可验证绘制范围和取消行为，不能证明 Kindle 实际刷新速度或残影表现；本版未做物理设备手感验收。
...
动画选择：阅读菜单 → 阅读设置 → 动画效果。点按依次切换：原版翻页、Swipe动画、擦除渐显、水波纹、侧边水波纹、聚拢水波纹、波浪推进、关闭。水波纹形态复用现有可取消调度、UI/Fast 刷新和横竖屏帧间隔；除固定 Swipe 预设外，跨章净屏开关开启时优先执行净屏动画。
...
`legado.koplugin` 在 Kindle KOReader 上独立运行。v0.10.31 未激活时最多添加 5 本书，添加第 6 本需输入短密钥；同一授权同时解锁阅读小票和阅读回顾数据展示，其他功能免费。已有书籍不会因升级被删除，重复加入同一本不占新名额，删除收藏后可腾出名额。首次激活联网绑定设备，之后可离线验证；沿用已发放的 Legado 短密钥。新安装不内置书源，需要从本地 JSON 或网址自行导入。网络书默认使用基于 Leko 的独立文字阅读器和侧边水波纹，兼容缺少 Fast 刷新接口的 Kindle。本地 EPUB/PDF 等文档仍交给 KOReader；独立阅读菜单点击“关闭无感阅读”可回到 KOReader。双向切换保留章节和近似章内位置，两种排版分别按书保存。阅读页目录新增侧边懒加载面板，每页 15 章，可放在左侧或右侧，支持目录、书签、字体和配置页。本版继续优化缓存命中章节切换，当前章节提交后立即后台预取下一章，并继续填充后两章；页脚“阅读进度”可独立启用/停用并保留显示模式。详情见 [v0.10.22 修复记录](docs/reader-library-0.10.22.md)。
...
2026-09-23 v0.10.23：修复 Kindle 首次创建旁路授权文件时将“文件不存在”误判为存储空间不足；保留授权收据、普通设置与既有离线授权。
...
插件不内置书源，也不自动下载或恢复书源。首次使用请进入“更多 → 书源管理”，选择“从本地 JSON 导入”或“从网址导入”，长按选择设备上的 JSON 文件，或填入自己的合集地址。导入后可正常聚合搜索、浏览发现分类和阅读。没有书源时，发现提示“请先导入书源”。升级保留设备上已经保存的书源、书架、进度、设置和授权；旧书源统一在管理页显示，可自行停用或删除。开发回归使用的书源样本不随安装包分发。
...
- 阅读回顾数据、阅读小票展示及超过 5 本的书架需要短密钥；搜索、书源导入、下载、原生/无感阅读及 KOReader 原生统计免费。本地目录文件列表保持正常扫描，书架记录的新增写入统一检查限额；已有书籍更新和阅读进度保存不受限制。未激活也正常保存阅读进度与时长，激活后可查看既有记录。
...
- 导入显示 `STORAGE_ERROR`：更新至 v0.10.29 后，先根据提示区分读取书源文件、读取数据库或保存数据库失败。新版按实际故障显示“保存本地数据库”、空间不足、只读或占用等原因，不再把保存失败解释为书源规则错误。请保留原数据库，不要通过删除数据来重试。
