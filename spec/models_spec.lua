...
local moved = Models.chapter(first, source, { index = 99, title = "Renamed", url = "../chapter/4#position" }, "https://books.test/toc/list")
equal(chapter.uid, moved.uid, "URL-backed chapter UID ignores title index and fragment")
local no_url_one = Models.chapter(first, source, { index = 1, title = "Fallback One" }, nil)
local no_url_two = Models.chapter(first, source, { index = 2, title = "Fallback Two" }, nil)
truthy(no_url_one.uid ~= no_url_two.uid, "URL-less chapters use title and index fallback identity")
local explicit_free = Models.chapter(first, source, { index = 5, title = "Free", vip = false, isVip = true }, nil)
equal(false, explicit_free.vip, "explicit vip=false wins over later aliases")
truthy(first.source_id ~= source.id, "model source id is opaque instead of a credential-bearing URL")
equal(nil, first.source_ref, "book model never exposes the raw source lookup key")
equal(nil, chapter.source_ref, "chapter model never exposes the raw source lookup key")
