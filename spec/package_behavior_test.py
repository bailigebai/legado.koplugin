...
def main() -> int:
    ...
    artifact = root / "dist" / "legado.koplugin-v0.10.33.zip"

    build = [powershell, "-ExecutionPolicy", "Bypass", "-File", str(package), "-Version", "0.10.33", "-SkipTests"]
    ...
    def verify_command(path: Path) -> list[str]:
        return [powershell, "-ExecutionPolicy", "Bypass", "-File", str(verify), "-Archive", str(path), "-Version", "0.10.33"]
    ...
    with tempfile.TemporaryDirectory(prefix="legado-package-test-") as temporary:
        ...
        def change_version(name: str, data: bytes):
            if name == "legado.koplugin/_meta.lua":
                data = data.replace(b'version = "0.10.33"', b'version = "9.9.9"')
            return name, data
