...
local jpeg = '\255\216\255\224test\255\217'
...
pending({ status = 200, headers = { ["Content-Type"] = "image/jpeg" }, body = jpeg }, nil)
assertx.equal(nil, failure, "valid cover load succeeds")
assertx.truthy(path:match("%.jpg$"), "content type selects a safe extension")
assertx.equal(jpeg, writes[1][2], "cover bytes are atomically written")
assertx.equal("function", type(handle.cancel), "cover loader returns the network cancel handle")

local cached_path=path
fs.readBounded=function(_, candidate) if candidate==cached_path then return jpeg end end
requests.execute=function() error("cached covers must work without network") end
path=nil
loader:load({id="book-secret-id",source_id="opaque-source",cover_url="https://covers.test/one"},function(value) path=value end)
assertx.equal(cached_path,path,"independent cover cache is reused")
return 7
