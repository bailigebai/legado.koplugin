...
插件设置单独保存在 `${DataStorage:getDataDir()}/settings/legado.lua`。书源、书架、目录、阅读进度和下载任务保存在 `${DataStorage:getDataDir()}/legado/legado.sqlite`；SQLite 不可用时，同一路径保存 Lua 降级索引。正文目录/章节缓存位于 `${DataStorage:getDataDir()}/legado/cache/`，封面位于 `${DataStorage:getDataDir()}/legado/covers/`，生成的 EPUB 与 `.part` 位于 `${DataStorage:getDataDir()}/legado/downloads/`。卸载插件目录不会自动删除这些数据或设置；删除前请自行备份。
...
4. 要完整清除个人数据与设置，必须同时删除 `${DataStorage:getDataDir()}/legado/` 和 `${DataStorage:getDataDir()}/settings/legado.lua`。前者清除书源 Header/凭据、书架、进度、任务、缓存、封面和 EPUB，后者清除插件设置；只删除 `legado/` 不会清除设置。
